"""
my own dataloader:
by changing something in __next__(),we can load more than 1 sample in batch to CNN training
(to make all the sample in batch has the same v1,v2)
"""
...
#######################################################################################
## change the method self.next
## to make all the sample in batch has the same v1,v2

class multiDataIter(object):
    ...
    def __next__(self):
        if self.num_workers == 0:  # same-process loading
            ...
            v2=random.randint(0,12)
            while (v1 == v2):
                v2 = random.randint(0, 12)  ## v1 != v2
            batch = self.collate_fn([self.dataset[(i,v1,v2)] for i in indices])
            if self.pin_memory:
                batch = pin_memory_batch(batch)
            return batch
